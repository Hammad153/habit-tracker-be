...
function anchoredWeek(weekNumber: number): {
  weekStart: string;
  weekEnd: string;
} {
...
async function main() {
    ...
    data: {
      ...
      completionRate: parseFloat(
        (
          ...
          Math.max(allCompletions.length, 1)
        ).toFixed(2),
      ),
    },
  });
  console.log(
    `✓ Stats: Level ${level}, ${stats.totalXp} XP, ${stats.ledgerCoins} coins, ${stats.longestStreak}d streak`,
  );

  const ledgerCount = await seedRewardLedger(
    user.id,
    habitRecords,
    allCompletions,
  );
  ...
  console.log(
    `✓ ${shopResult.items} shop items, ${shopResult.redemptions} redemptions`,
  );
...
async function createHabits(userId: string) {
  ...
  for (const config of HABIT_CONFIGS) {
    const createdAt = config.createdDaysAgo
      ? daysAgo(config.createdDaysAgo)
      : daysAgo(42);
    const habit = await prisma.habit.create({
      data: {
        ...
        fullBehavior: config.fullBehavior,
        minimumBehavior: config.minimumBehavior,
        emergencyMinimum: config.emergencyMinimum,
        scheduleType: config.scheduleType,
        scheduleDays: config.scheduleDays || [],
        streakBonusEnabled: true,
        identityBonusEnabled: true,
        rewardFundAmount: 5,
        createdAt,
      },
    });
...
function generateCompletions(
  habits: Array<{ id: string; config: HabitConfig; createdAt: Date }>,
) {
...
function computeStats(
  ...
  for (const habit of habits) {
    ...
    const fullDates = completions
      .filter((c) => c.habitId === habit.id && c.kind === 'FULL')
      .map((c) => c.date);
  ...
  return {
    ...
    totalXp,
    longestStreak,
    ledgerCoins: totalCoins + milestoneCoins,
  };
}
...
async function seedDailyPlans(
  ...
  const plans: Array<{
  ...
  }> = [
    {
      ...
      tasks: [
        ...
        },
        {
          ...
          description: 'Focus on the main project deliverable',
          priority: 'HIGH',
          status: 'COMPLETED',
          habitId: deepWork?.id,
          startTime: '09:00',
        ...
        },
        {
          ...
          description: 'Reflect on the day and write gratitude',
          priority: 'LOW',
          status: 'PENDING',
          habitId: journal?.id,
          startTime: '21:30',
        ...
        },
        {
          ...
          description: "Check progress on this week's objectives",
          priority: 'LOW',
          status: 'PENDING',
          habitId: null,
          startTime: '22:00',
    ...
    },
    {
      ...
      note: 'Prepare for a productive day.',
      tasks: [
        {
          title: 'Morning meditation',
          description: 'Start the day with 10 minutes of calm',
          priority: 'HIGH',
          status: 'PENDING',
          habitId: meditation?.id,
          startTime: '06:00',
        ...
        {
          title: 'Deep work: project deadline',
    ...
    },
    {
      ...
      note: 'Reflected on progress made.',
      tasks: [
        {
          title: 'Morning meditation',
        ...
        {
          title: 'Deep work session',
          description: 'Backend feature implementation',
          priority: 'HIGH',
          status: 'COMPLETED',
          habitId: deepWork?.id,
          startTime: '09:00',
        ...
        },
        {
          ...
          description: "Write about today's wins",
          priority: 'LOW',
          status: 'COMPLETED',
          habitId: journal?.id,
          startTime: '21:30',
  ...
  const entries = [
    {
      ...
      date: dateToStr(daysAgo(40)),
      mood: 'happy' as const,
      content:
        'Today was the first day of my new habit tracking journey. I set up all my habits and felt genuinely excited about the changes I want to make. The morning workout was tough but I pushed through. Drinking water consistently felt easier than I expected.\n\nOne thing I noticed is that I tend to forget about habits after lunch. I should set more reminders for the afternoon ones. But overall, a very positive start.',
      tags: ['milestone', 'beginning'],
      isFavorite: false,
      isPinned: false,
    },
    {
      title: 'Learning to Be Consistent',
      date: dateToStr(daysAgo(35)),
      mood: 'focused' as const,
      content:
        "Day 5 and I'm starting to feel the rhythm. The morning routine is becoming more automatic — wake up, meditate, workout, water. I missed the journaling yesterday though. I was just too tired by 9:30 PM.\n\nI realized that the key isn't perfection, it's showing up most days. Even on days where I can only do the minimum version of a habit, that counts. The streak counter is really motivating — I don't want to break it.",
      tags: ['consistency', 'reflection'],
      isFavorite: true,
      isPinned: false,
    },
    {
      title: 'A Difficult Day',
      date: dateToStr(daysAgo(30)),
      mood: 'stressed' as const,
      content:
        "Work was overwhelming today. Deadline moved up by a week and I had to scramble. Skipped my workout entirely and only managed the emergency minimum for most habits. Drank water but that's about it.\n\nI'm trying not to beat myself up about it. The emergency minimum exists for exactly these kinds of days. At least I didn't skip everything. Tomorrow I'll get back on track.",
...
async function seedRewardShop(userId: string) {
  const itemsData = [
    {
      ...
      name: 'Golden Theme',
      description:
        'A luxurious golden interface theme that transforms the entire app experience.',
      cost: 500,
      type: 'THEME' as const,
    },
    {
      ...
      name: 'Focus Theme',
      description:
        'A calm, low-distraction theme designed for deep work sessions.',
      cost: 300,
      type: 'THEME' as const,
    },
    {
      ...
      cost: 200,
      type: 'JOURNAL_THEME' as const,
    },
    {
      key: 'avatar-frame-golden',
      name: 'Golden Avatar Frame',
      description:
        'Show off your dedication with a shining golden profile frame.',
      cost: 250,
    ...
    },
    {
      ...
      description:
        'Confetti and fanfare effects for every milestone you unlock.',
      cost: 150,
    ...
    },
    {
      ...
      description:
        'A refreshing ocean-inspired theme with cool blues and teals.',
      cost: 350,
      type: 'THEME' as const,
    },
    {
      key: 'avatar-neon',
      name: 'Neon Avatar Glow',
      description: 'A vibrant neon glow effect for your profile avatar.',
      cost: 200,
      type: 'AVATAR' as const,
    },
    {
      ...
      description:
        'An ultra-dark theme for night owls with eye-friendly contrast.',
      cost: 400,
      type: 'THEME' as const,
    },
  ];
  ...
  const celebration = await prisma.rewardItem.findUnique({
    where: { key: 'pack-celebration' },
  });
  if (celebration) {
    await prisma.rewardRedemption.upsert({
      where: { userId_itemId: { userId, itemId: celebration.id } },
      update: {},
      create: { userId, itemId: celebration.id, cost: celebration.cost },
    });
  ...
  const journalTheme = await prisma.rewardItem.findUnique({
    where: { key: 'theme-journal' },
  });
  if (journalTheme) {
    await prisma.rewardRedemption.upsert({
      where: { userId_itemId: { userId, itemId: journalTheme.id } },
      update: {},
      create: { userId, itemId: journalTheme.id, cost: journalTheme.cost },
    });
    await prisma.rewardLedger.upsert({
      ...
      create: {
        userId,
        amount: -journalTheme.cost,
        type: 'REWARD_REDEMPTION',
        referenceType: 'REWARD_ITEM',
        referenceId: journalTheme.id,
  ...
  return {
    ...
    totalSpent: totalRedemptionCost,
  };
}
...
async function seedBudgets(userId: string) {
  ...
  // Allocations
  const allocs = [
    { cat: 'Food & Dining', amount: 800 },
    { cat: 'Transportation', amount: 300 },
    { cat: 'Entertainment', amount: 200 },
    { cat: 'Health & Fitness', amount: 250 },
    { cat: 'Education', amount: 150 },
    { cat: 'Shopping', amount: 300 },
    { cat: 'Utilities', amount: 350 },
    { cat: 'Subscriptions', amount: 150 },
  ];
  const allocData = allocs
    .map((a) => ({
      budgetId: budget.id,
  ...
  await prisma.income.createMany({
    data: [
      { userId, title: 'Monthly salary', amount: 3500, incomeDate: daysAgo(5) },
      {
        ...
        amount: 500,
        incomeDate: daysAgo(12),
      },
      {
        userId,
        title: 'Sold old laptop',
        amount: 200,
        incomeDate: daysAgo(20),
      },
    ],
  });
  count += 3;
  ...
  const data = [
    {
      ...
      description:
        'I am someone who prioritizes physical health and strength. Exercise is not optional — it is part of who I am.',
      icon: 'barbell',
      color: '#EF4444',
      linked: ['Morning Workout', 'Walk 8,000 Steps'],
    },
    {
      title: 'Lifelong Learner',
      description:
        'I am committed to continuous growth and knowledge. Reading and learning new skills are fundamental to my daily routine.',
      icon: 'book',
      color: '#F59E0B',
      linked: ['Read 30 Minutes', 'Learn Arabic Vocabulary'],
    },
    {
      title: 'Mindful Individual',
      description:
        'I cultivate awareness and presence in everything I do. Meditation and reflection are the foundations of my mental clarity.',
      icon: 'flower',
      color: '#A855F7',
      linked: ['Meditate', 'Journal'],
    },
  ];
  ...
  for (const badge of badges) {
    ...
    await prisma.userBadge.upsert({
      ...
      create: {
        userId,
        badgeId: badge.id,
        earnedAt: daysAgo(Math.floor(Math.random() * 20) + 5),
      },
    });
    count++;
  }
...
async function seedNotifications(userId: string) {
  const notifs = [
    {
      fingerprint: 'notif-streak-milestone',
      type: 'STREAK_MILESTONE',
      status: 'SURFACED',
      priority: 'HIGH',
      dayKey: dateToStr(daysAgo(1)),
    },
    {
      fingerprint: 'notif-habit-reminder',
      type: 'HABIT_REMINDER',
      status: 'SURFACED',
      priority: 'MEDIUM',
      dayKey: dateToStr(new Date()),
    },
    {
      fingerprint: 'notif-daily-progress',
      type: 'DAILY_PROGRESS',
      status: 'SURFACED',
      priority: 'LOW',
      dayKey: dateToStr(new Date()),
    ...
    {
      fingerprint: 'notif-achievement-unlocked',
      type: 'ACHIEVEMENT_UNLOCKED',
      status: 'SURFACED',
      priority: 'HIGH',
      dayKey: dateToStr(daysAgo(3)),
    },
    {
      fingerprint: 'notif-weekly-review',
      type: 'WEEKLY_REVIEW',
      status: 'SURFACED',
      priority: 'MEDIUM',
      dayKey: dateToStr(daysAgo(2)),
    },
    {
      fingerprint: 'notif-encouragement',
      type: 'ENCOURAGEMENT',
      status: 'EXPIRED',
      priority: 'LOW',
      dayKey: dateToStr(daysAgo(5)),
    },
    {
      fingerprint: 'notif-new-habit-tip',
      type: 'HABIT_TIP',
      status: 'SURFACED',
      priority: 'LOW',
      dayKey: dateToStr(daysAgo(1)),
  ...
  const reviewSpecs = [
    {
      ...
      summary:
        'Great first week! Established morning routine and maintained consistency across most habits.',
      wins: [
        'Completed 5/7 workout sessions',
        'Meditated every day',
        'Drank 3L water 6/7 days',
      ],
      patterns: [
        'Morning habits are easier to maintain than evening ones',
        'Energy drops significantly after 9 PM',
      ],
      identityReflection:
        'You are becoming someone who shows up for the basics — water, movement, and a calm morning. Week one built the identity of a consistent person.',
      nextWeekFocus: [
        'Protect the morning routine',
        'Shift one evening habit to the morning',
      ],
    },
    {
      ...anchoredWeek(3),
      ...
      summary:
        'Second week showed improvement in consistency. Deep work sessions became more productive.',
      wins: [
        'Perfect deep work week',
        'Finished first book',
        'Walking habit reached 8000 steps 5/7 days',
      ],
      patterns: [
        'Deep work is most productive between 9-11 AM',
        'Exercise improves sleep quality when done before 6 PM',
      ],
      identityReflection:
        'Your identity as a reader is starting to solidify — finishing the first book is quiet but powerful evidence.',
      nextWeekFocus: [
        'Keep a fixed deep work block',
        'Move exercise before 6 PM on weekdays',
      ],
    },
    {
      ...anchoredWeek(4),
      ...
      summary:
        'A challenging week with some missed days. Work deadline caused stress. Emergency minimum prevented total streak loss.',
      wins: [
        'Used emergency minimum instead of skipping',
        'Rebounded strongly on Friday',
        'Journaling helped process stress',
      ],
      patterns: [
        'Stress correlates with missed evening habits',
        'Emergency minimum is a lifesaver during busy weeks',
      ],
      identityReflection:
        'A week of deadlines did not break who you are becoming. Using the emergency minimum proved the habit identity holds under pressure.',
      nextWeekFocus: [
        'Plan journaling before high-stress days',
        'Rebound plan for missed evenings',
      ],
    },
    {
      ...anchoredWeek(5),
      ...
      summary:
        "Bounced back from last week's difficulties. Added Arabic vocabulary as a new habit. Deep work sessions hit peak productivity.",
      wins: [
        'New Arabic habit started strong',
        '7 consecutive workout days',
        'Reading streak reached 14 days',
      ],
      patterns: [
        'Adding new habits reinvigorates motivation',
        'Consistent bedtime improves morning energy',
      ],
      identityReflection:
        'You are building a multilingual, physically consistent identity — the two-week streak is your strongest evidence yet.',
      nextWeekFocus: [
        'Maintain Arabic vocabulary streak',
        'Keep the 9:30 PM bedtime',
      ],
    },
    {
      ...anchoredWeek(6),
      ...
      summary:
        'The best week so far! Completed all habits on 3 out of 7 days. Meditation breakthrough. Identity shift toward "fitness person" is real.',
      wins: [
        '3 perfect days in one week',
        'Meditation breakthrough',
        'Longest water tracking streak: 14 days',
        'Earned Centurion badge (100 completions)',
      ],
      patterns: [
        'Perfect days cluster around low-stress workdays',
        'Morning routine completion predicts overall daily success',
      ],
      identityReflection:
        'You are a fitness person now — not hoping to become one. Three perfect days and a 14-day water streak are the evidence.',
      nextWeekFocus: [
        'Aim for 4 perfect days',
        'Improve sleep consistency',
        'Protect the 100-completion badge momentum',
      ],
    },
  ];
...
async function seedTemptationBundles(
  userId: string,
  habits: Array<{ id: string; title: string }>,
) {
  const bundles = [
    {
      ...
      title: 'Post-Workout Treat',
      description:
        'After completing 7 consecutive workout sessions, unlock a special reward.',
      status: 'UNLOCKED' as const,
    },
    {
      habitTitle: 'Read 30 Minutes',
      title: 'Bookworm Bonus',
      description: 'Read every day for a week and earn bonus coins.',
      status: 'LOCKED' as const,
    },
    {
      habitTitle: 'Deep Work Session',
      title: 'Flow State Reward',
      description:
        'Complete 5 deep work sessions in a row. Your productivity deserves recognition.',
